...
from notifications.models import (EmailSendStatus, Notification, SmsSendStatus,
...
def create_users(users):
    ...
    for user in users:
        User.objects.create(
            name=user[0], email=user[1], number=user[2], tg_chat_id=user[3]
        )
    ...
    for user in users:
        try:
            ...
            EmailSendStatus.objects.create(
                user=user, is_successful=False, error_message=str(e)
            )
    ...
    for emails_status in failed_email_statuses:
        ...
        if response.status_code == 200:
            TelegramSendStatus.objects.create(
                user=emails_status.user, is_successful=True
            )
        else:
            TelegramSendStatus.objects.create(
                user=emails_status.user,
                is_successful=False,
    ...
    for telegram_status in telegram_send_status_false:
        try:
            ...
            SmsSendStatus.objects.create(
                user=telegram_status.user, is_successful=False, error_message=e
            )
