...
from notifications.models import (EmailSendStatus, SmsSendStatus,
                                  TelegramSendStatus, User)
...
def users():
    user1 = User.objects.create(
        name="Tamir",
        email="tamirmandreev@mail.ru",
        number="+79915410704",
        tg_chat_id="747451276",
    )
    user2 = User.objects.create(
        name="Timur",
        email="mandreevts@gmail.com",
        number="+79644111469",
        tg_chat_id="747451276",
    )
...
class TestSendTelegramMessage:
    ...
    def failed_email_statuses(self, users):
        ...
        failed_email_statuse1 = EmailSendStatus.objects.create(
            user=users[0], is_successful=False
        )
        failed_email_statuses2 = EmailSendStatus.objects.create(
            user=users[1], is_successful=False
        )
...
class TestSendSmsMessage:
    ...
    def failed_telegram_statuses(self, users):
        ...
        failed_telegram_statuse1 = TelegramSendStatus.objects.create(
            user=users[0], is_successful=False
        )
        failed_telegram_statuses2 = TelegramSendStatus.objects.create(
            user=users[1], is_successful=False
        )
    ...
    def test_successful_sms_send(self, mock_sms_aero, failed_telegram_statuses):
        ...
        mock_sms_aero.assert_called_once_with(
            settings.SMSAERO_EMAIL, settings.SMSAERO_API_KEY
        )
...
class TestGenerateNotificationReport:
    ...
    def objects(self, users):
        ...
        EmailSendStatus.objects.create(
            user=users[1], is_successful=False, error_message="Тестовая ошибка 1"
        )
        EmailSendStatus.objects.create(
            user=users[2], is_successful=False, error_message="Тестовая ошибка 2"
        )
        EmailSendStatus.objects.create(
            user=users[3], is_successful=False, error_message="Тестовая ошибка 3"
        )
        TelegramSendStatus.objects.create(user=users[1], is_successful=True)
        TelegramSendStatus.objects.create(
            user=users[2], is_successful=False, error_message="Тестовая ошибка 4"
        )
        TelegramSendStatus.objects.create(
            user=users[3], is_successful=False, error_message="Тестовая ошибка 5"
        )
        SmsSendStatus.objects.create(user=users[2], is_successful=True)
        SmsSendStatus.objects.create(
            user=users[3], is_successful=False, error_message="Тестовая ошибка 6"
        )
